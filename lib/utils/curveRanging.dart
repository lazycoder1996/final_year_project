...
Widget computeHorizontalCurveParameters(
    {String scenario, Map<String, dynamic> computationalValues}) {
  ...
  try {
    ...
    if (scenario == "Scenario 1" ||
      ...
      lengthOfCurve = num.parse(
          ((pi * deflectionAngleValue * radiusValue) / 180).toStringAsFixed(3));
      ...
      // Long chord
      longChord = num.parse((2 * radiusValue * sin(halfDeflection * converter))
          .toStringAsFixed(3));
    ...
    if (scenario == "Scenario 1" || scenario == "Scenario 4") {
      ...
      tangentialAngles.add(((initialSubchord * constant) / (radiusValue * 60)));
      ...
      try {
        while (n < numberOfChords) {
          deflectionAngles.add((deflectionAngles[n] + tangentialAngles[n + 1]));
          n++;
        }
      } catch (e) {}
    ...
    if (scenario == "Scenario 4") {
      ...
      while (n < size) {
        ...
        lengthsSetupStationToPointsOnCurve.add(num.parse(a.toStringAsFixed(3)));
      ...
      columns = ["Point", "Eastings", "Northings", "Bearing Xp", "Distance Xp"];
    }
    if (scenario == "Scenario 1") {
      dataRows = [];
      int size = chordLengths.length;
...
Widget computeVerticalCurveParameters(
    {String scenario, Map<String, dynamic> computationalValues}) {
  return null;
}
